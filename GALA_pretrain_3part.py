...
def run(local_rank, nprocs, devices, partition_res, args):
    ...
    if local_rank == 0:
        ...
        torch.save(model.state_dict(), f'GALA_trainres/{args.model}_{args.dataset}_combo{args.gpus}_noCASE_3p.pth' )
...
def main():
    ...
    parser.add_argument("--delta0", type=str, default='0.12')
    parser.add_argument("--delta1", type=str, default='0.33')
    parser.add_argument("--delta2", type=str, default='0.55')
    ...
    gpus = ['5,4,0', '5,0,2', '4,0,2', '5,4,6'][args.gpus]
    ...
    sg0 = dgl.load_graphs(f"/mnt/disk/hjf/NESA_3part/{args.dataset}_sg0_{args.delta0}+{args.delta1}+{args.delta2}.bin")[0][0]
    sg1 = dgl.load_graphs(f"/mnt/disk/hjf/NESA_3part/{args.dataset}_sg1_{args.delta0}+{args.delta1}+{args.delta2}.bin")[0][0]
    sg2 = dgl.load_graphs(f"/mnt/disk/hjf/NESA_3part/{args.dataset}_sg2_{args.delta0}+{args.delta1}+{args.delta2}.bin")[0][0]
    partition_res = [sg0, sg1, sg2]
